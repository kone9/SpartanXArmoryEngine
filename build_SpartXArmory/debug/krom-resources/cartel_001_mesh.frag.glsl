...
void main()
{
    ...
    vec4 ImageTexture_001_texread_store = texture(ImageTexture_001, vec2(Mapping_texread_Vector_res.x, 1.0 - Mapping_texread_Vector_res.y));
    vec3 _105 = pow(ImageTexture_001_texread_store.xyz, vec3(2.2000000476837158203125));
    ImageTexture_001_texread_store = vec4(_105.x, _105.y, _105.z, ImageTexture_001_texread_store.w);
    vec4 ImageTexture_texread_store = texture(ImageTexture, texCoord);
    vec3 _115 = pow(ImageTexture_texread_store.xyz, vec3(2.2000000476837158203125));
    ImageTexture_texread_store = vec4(_115.x, _115.y, _115.z, ImageTexture_texread_store.w);
    ...
    vec3 Mix_Result_res = mix(ImageTexture_Color_res, ImageTexture_Color_res * vec3(0.846834003925323486328125, 0.0, 0.0), vec3(1.0));
    vec3 basecol = ImageTexture_001_Color_res;
    float roughness = 0.0764710009098052978515625;
    float metallic = 1.0;
    float occlusion = 1.0;
    float specular = 0.5;
    vec3 emissionCol = Mix_Result_res * 1.0;
